...
func part1() {
	board := make([][]int, len(boards)/25)
	for i := range board {
		board[i] = boards[i*25 : (i+1)*25]
	}

	for _, number := range rand {
		for i := range boards {
			if boards[i] == number {
				boards[i] = -number
			}
...
func part2() {
	...
	won := make([]bool, len(board))
	unwonCount := len(board)
...
func wins(board []int) bool {
	return marked(board[0:], 1) ||
		marked(board[5:], 1) ||
		marked(board[10:], 1) ||
		marked(board[15:], 1) ||
		marked(board[20:], 1) ||
...
func marked(b []int, inc int) bool {
	for i := 0; i < 5; i++ {
		if b[i*inc] >= 0 {
			return false
		}
